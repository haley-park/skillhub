...
export default function SkillDetailPage() {
  ...
  async function handleOpen(target: "editor" | "folder") {
    try {
      ...
      toast(target === "editor" ? "에디터에서 열었어요" : "폴더를 열었어요", "success");
    } catch {
      toast("열기에 실패했어요", "error");
    }
  }
    ...
    return (
      <div className="min-h-screen bg-bg">
        ...
        <main className="max-w-container mx-auto px-6 md:px-8 py-24 text-center">
          ...
          <h1 className="text-h1 text-text-primary mb-2">스킬을 찾을 수 없어요</h1>
          <p className="text-body text-text-secondary mb-6">
            &ldquo;{skillName}&rdquo; 스킬이 존재하지 않거나 삭제되었어요
          </p>
          <Link href="/">
  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <main className="max-w-container mx-auto px-6 md:px-8 py-12">
        {/* Breadcrumb */}
        <nav aria-label="경로" className="flex items-center gap-2 text-caption text-text-tertiary mb-8">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <time dateTime={skill.last_modified} className="text-caption text-text-tertiary">
                    {relativeTime(skill.last_modified)} 수정
                  </time>
                </div>
              </div>
            </div>
